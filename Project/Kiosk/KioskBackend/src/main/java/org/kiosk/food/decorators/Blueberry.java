...
public class Blueberry extends IngridientDecorator {
    private static String ingredientName;
    private static int ingredientPrice;
    private static ArrayList<FoodType> foodTypes;
    public Blueberry(IFood food) throws ClassNotFoundException {
        super(food);
        initialize();
    }

    private static void initialize() throws ClassNotFoundException {
        ingredientName = (Class.forName("org.kiosk.food.decorators.Blueberry")).getName().split("\\.")[4];
        ingredientPrice = 350;
        foodTypes = new ArrayList<FoodType>();
        foodTypes.add(FoodType.Shake);
    }
